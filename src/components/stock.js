...
const StockDetailsPage = () => {
  ...
  const filteredData = medicineData
    .filter(
      (item) =>
        item.medicinename &&
        item.medicinename.toLowerCase().includes(searchQuery.trim().toLowerCase()) &&
        (!fromExpiryDate ||
          moment(item.expirydate).isSameOrAfter(fromExpiryDate)) &&
        (!toExpiryDate || 
          moment(item.expirydate).isSameOrBefore(toExpiryDate))
    )
    .sort((a, b) => {
  ...
  const handleSaveEdit = async (id, updatedData) => {
    try {
      ...
      setSuccessAlert(
        <div class="top-15 text-white p-2 rounded"  
         style={{ backgroundColor: "rgb(72, 194,205)", width:'250px', marginLeft:'350px' }}>
    ...
    if (window.confirm("Are you sure you want to delete this item?")) {
      try {
        ...
        if (response.status === 200) {
          ...
          setSuccessAlert(
            <div class="top-15 text-white p-2 rounded"  
            style={{ backgroundColor: "orange", width:'250px', marginLeft:'350px' }}>
